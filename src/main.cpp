...
void timerClock(lv_timer_t *timer){
...
void timerBuzzer(lv_timer_t *timer){
...
void setup() {
  ledcSetup(LCD_PWMCHANNEL, 5000/*freq*/, 10 /*resolution*/);
  ledcAttachPin(32, LCD_PWMCHANNEL);
  analogReadResolution(10);
  ledcWrite(LCD_PWMCHANNEL, 112);
...
// lv_fs_dir_close(&dir);
  ...
  lv_timer_t * timer = lv_timer_create(timerClock, 1000, &user_data);
...
void showObjTop() {
	...
	lv_style_set_bg_color(&styleTopBar, lv_color_white());
	lv_style_set_opa(&styleTopBar, LV_OPA_COVER);
	lv_style_set_radius(&styleTopBar, 0);
	
	bckTop = lv_obj_create(lv_scr_act(), NULL);
	lv_obj_add_style(bckTop, LV_PART_MAIN, LV_STATE_DEFAULT, &styleTopBar);
	lv_obj_set_pos(bckTop, 0, 0);
	//lv_obj_align(bckTop, NULL, LV_ALIGN_IN_BOTTOM_MID, 0, 0);
	...
	lv_label_set_text(lblClock, "12:00:00");
	...
	lv_obj_set_style_text_align(lblDate, LV_PART_MAIN, LV_STATE_DEFAULT, LV_TEXT_ALIGN_CENTER);
...
void showObjMain() {
	...
	lv_style_set_bg_color(&styleMenu, lv_color_white());
	lv_style_set_opa(&styleMenu, LV_OPA_COVER);
	lv_style_set_radius(&styleMenu, 0);

	bckMain = lv_obj_create(lv_scr_act(), NULL);
	lv_obj_add_style(bckMain, LV_PART_MAIN, LV_STATE_DEFAULT, &styleMenu);
	lv_obj_set_pos(bckMain, 0, 30);
	lv_obj_set_height(bckMain, 320 - 60);
	lv_obj_set_width(bckMain, 480);
}
void showObjBottom() {
	bckBottom = lv_obj_create(lv_scr_act(), NULL);
	lv_obj_set_pos(bckBottom, 0, 290);
	//lv_obj_align(bckBottom, NULL, LV_ALIGN_IN_BOTTOM_MID, 0, 0);
	lv_obj_set_size(bckBottom, 480, 30);

	btnBack = lv_btn_create(bckBottom, NULL);
	lv_obj_add_event_cb(btnBack, NavButton_cb, NULL);
	lv_obj_align(btnBack, NULL, LV_ALIGN_CENTER, 0, 0);
	lv_obj_set_size(btnBack, 40, 30);
	//lv_btn_set_fit(btnBack, LV_FIT_TIGHT);
	lv_obj_set_style_border_opa(btnBack, LV_PART_ANY, LV_STATE_ANY, 0);
...
static void Menu_cb(lv_obj_t* obj, lv_event_t event) {
	if (event == LV_EVENT_VALUE_CHANGED && obj == btnMenus) {
		uint16_t idBtn = lv_btnmatrix_get_selected_btn(obj);
...
// 		MenuScreen();
// 	}
// 	else if (event == LV_EVENT_CLICKED && lv_list_get_btn_text(listWiFi, btnWiFis) != NULL) {
// 		const char* btntxts[] = {"OK", "Cancel"};

// 		msgboxWiFi = lv_msgbox_create("WiFi","Insert password",btntxts, false);
...
// 		lv_textarea_set_password_show_time(textarea, 300);
...
// 		lv_obj_add_event_cb(msgboxWiFi, WiFi_cb);
...
static void Settings_cb(lv_obj_t* obj, lv_event_t event) {
	if (obj == sliderBrightness && event == LV_EVENT_VALUE_CHANGED) {
		...
		lv_msgbox_create("ID Kartu", "ID kartu adalah: ", btnClose, false);
...
void TrackingScreen_2() {
	...
	lv_obj_add_state(cbxScanID, LV_STATE_DISABLED);
...
void LoginScreen_2() {
  ...
  showObjBottom();
	...
	lv_obj_set_style_text_font(lblLogin, LV_PART_ANY, LV_STATE_ANY, &lv_font_montserrat_14);
	lv_obj_set_style_text_align(lblLogin, LV_PART_ANY, LV_STATE_ANY, LV_TEXT_ALIGN_CENTER);
...
void MenuScreen() {
  ...
  lv_style_set_opa(&styleBGMenus, 0);

	lv_style_init(&styleBtnMenus);
  lv_style_set_opa(&styleBtnMenus, LV_OPA_COVER);
  lv_style_set_text_font(&styleBtnMenus, &lv_font_montserrat_12);
	...
	lv_obj_add_style(btnMenus, LV_PART_MAIN, LV_STATE_ANY, &styleBGMenus);
	lv_obj_add_style(btnMenus, LV_PART_ITEMS, LV_STATE_ANY, &styleBtnMenus);
	...
	lv_obj_add_event_cb(btnMenus, Menu_cb, NULL);

	lv_style_init(&styleLabelSatpam);
	lv_style_set_text_font(&styleLabelSatpam, &lv_font_montserrat_12);

	lblNama = lv_label_create(bckBottom, NULL);
	lv_obj_add_style(lblNama, LV_PART_MAIN, LV_STATE_ANY, &styleLabelSatpam);
	...
	lv_obj_add_style(lblNIP, LV_PART_MAIN, LV_STATE_ANY, &styleLabelSatpam);
...
void TapScreen() {
	...
	lv_obj_add_event_cb(btnTapLanjut, Tap_cb, NULL);
	...
	lv_obj_add_event_cb(btnTapProblem, Tap_cb, NULL);
...
void WiFiScreen() {
	...
	listWiFi = lv_list_create(bckMain);
	...
	for (int i = 0; i < 35; i++) {
		...
		btnWiFis = lv_list_add_btn(listWiFi, LV_SYMBOL_WIFI, txtFmt, WiFi_cb);
		//lv_obj_add_event_cb(btnWiFis, WiFi_cb, NULL);
	}
	...
	lv_obj_add_event_cb(btnBack, WiFi_cb, NULL);
}
...
void TrackingScreen() {
	...
	tableTracking = lv_table_create(bckMain, NULL);
	...
	lv_obj_set_style_text_align(tableTracking, LV_PART_ANY, LV_STATE_ANY, LV_TEXT_ALIGN_CENTER);
...
void PeminjamanScreen() {
	...
	tabviewPeminjaman = lv_tabview_create(bckMain, LV_DIR_VER, LV_COORD_MAX);
	...
	lv_obj_add_event_cb(tabviewPeminjaman, Peminjaman_cb, NULL);
...
void SettingsScreen() {
	...
	lv_obj_add_event_cb(sliderBrightness, Settings_cb, NULL);
	...
	lv_obj_add_event_cb(sliderBuzzer, Settings_cb, NULL);
	...
	lv_obj_add_event_cb(btnCheckID, Settings_cb, NULL);
...
void LogoutScreen() {
	...
	lv_obj_t *msgBox = lv_msgbox_create("Logout", "Apakah anda yakin untuk logout?", btns, false);
	lv_obj_set_width(msgBox, 200);
	lv_obj_align(msgBox, NULL, LV_ALIGN_CENTER, 0, 0);
	lv_obj_add_event_cb(msgBox, Logout_cb, NULL);
